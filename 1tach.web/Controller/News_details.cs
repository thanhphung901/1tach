...
namespace Controller
{
    public class News_details
    {
        ...
        public string Showfilehtm1(string _sCat_Seo_Url, string _sNews_seo_url)
        {
            try
            {
                string _result = string.Empty;
                ...
                if (_newid != 0)
                {
                    ...
                    if (s.Count > 0)
                    {
                        var _tinTucKhac = (from c in db.ESHOP_NEWS_CATs
                                           ...
                                           select new { a.NEWS_ID, a.NEWS_TITLE, a.NEWS_IMAGE3, a.NEWS_PRICE1, a.NEWS_DESC, a.NEWS_SEO_URL, a.NEWS_URL, a.NEWS_ORDER, a.NEWS_ORDER_PERIOD, a.NEWS_PUBLISHDATE }).Distinct().OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER).Take(10).Distinct();
        ...
        public List<News_details_entity> Load_hinhanh(int _newid)
        {
            try
            {
                List<News_details_entity> l = new List<News_details_entity>();
                if (_newid != 0)
                {
                    ...
                    if (s.Count > 0)
                    {
                        var hinhanh = (from c in db.ESHOP_NEWS_CATs
                                           join a in db.ESHOP_NEWs on c.NEWS_ID equals a.NEWS_ID
                                           join b in db.ESHOP_CATEGORies on c.CAT_ID equals b.CAT_ID
                                           where b.CAT_ID == s[0].CAT_ID
                                           select new {b.CAT_ID,b.CAT_IMAGE1,b.CAT_NAME }).Distinct();
                        if (hinhanh.ToList().Count > 0)
                        {
                            foreach (var i in hinhanh)
                            {
                                News_details_entity pro = new News_details_entity();
                                pro.CAT_ID = i.CAT_ID;
                                pro.CAT_IMAGE1 = i.CAT_IMAGE1;
       ...
       //Insert comment news detail
        ...
        public string Get_News_seo_url(string _sCatSeoUrl)
        {
            try
            {
                ...
                var list = (from a in db.ESHOP_NEWs
                            ...
                            select new { a.NEWS_SEO_URL,b.ESHOP_CATEGORy.CAT_ID }).ToList();
                if (list.Count > 0)
                {
        ...
        //    }
        //}
        public List<Entity_title> getday(string _sNews_Seo_Url)
        {
            try
            {
                List<Entity_title> l = new List<Entity_title>();
                ...
                var list = (from a in db.ESHOP_NEWS_CATs
                            ...
                            join c in db.ESHOP_CATEGORies on a.CAT_ID equals c.CAT_ID
                            where c.CAT_SEO_URL == _sNews_Seo_Url
                            select new { b.NEWS_PUBLISHDATE }).ToList();
                if (list.Count > 0)
                {
                    foreach (var i in list)
                    {
                        Entity_title t = new Entity_title();
                        t.ngay = Utils.CDateDef(i.NEWS_PUBLISHDATE, DateTime.Now);
                        l.Add(t);
                    }
                }
                else
